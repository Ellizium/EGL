...
namespace EGL.GodotBase.Utility
{
    public class Timer : INode
    {
        ...
        public Godot.Timer Base { get; }

        public Timer()
        {
            Base = new _Timer(this);
        }

        protected class _Timer : Godot.Timer
        {
            public Timer ClassOwner;

            public _Timer(Timer node)
            {
                ClassOwner = node;
