...
namespace EGL.GodotBase.Nodes2D.Utility
{
    public class Bone2D : INode
    {
        ...
        public Godot.Bone2D Base { get; }

        public Bone2D()
        {
            Base = new _Bone2D(this);
        }

        private class _Bone2D : Godot.Bone2D
        {
            public Bone2D ClassOwner;

            public _Bone2D(Bone2D node)
            {
                ClassOwner = node;
