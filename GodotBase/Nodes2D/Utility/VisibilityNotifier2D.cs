...
namespace EGL.GodotBase.Nodes2D.Utility
{
    public class VisibilityNotifier2D : INode
    {
        ...
        public event Action<Godot.Viewport> ViewportEntered;
        public event Action<Godot.Viewport> ViewportExited;

        public Godot.VisibilityNotifier2D Base { get; }

        public VisibilityNotifier2D()
        {
            Base = new _VisibilityNotifier2D(this);
        }

        private class _VisibilityNotifier2D : Godot.VisibilityNotifier2D
        {
            public VisibilityNotifier2D ClassOwner;

            public _VisibilityNotifier2D(VisibilityNotifier2D node)
            {
                ClassOwner = node;
                ...
                //VisibilityNotifier2D
                Connect("screen_entered", this, nameof(ScreenEntered));
                Connect("screen_exited", this, nameof(ScreenExited));
                Connect("viewport_entered", this, nameof(ViewportEntered));
                Connect("viewport_exited", this, nameof(ViewportExited));
            }
            ...
            #region VisibilityNotifier2D
            private void ScreenEntered()
            {
                ClassOwner.ScreenEntered?.Invoke();
            }
            private void ScreenExited()
            {
                ClassOwner.ScreenExited?.Invoke();
            }
            private void ViewportEntered(Godot.Viewport viewport)
            {
                ClassOwner.ViewportEntered?.Invoke(viewport);
            }
            private void ViewportExited(Godot.Viewport viewport)
            {
                ClassOwner.ViewportExited?.Invoke(viewport);
            }
            #endregion
        }
    }
