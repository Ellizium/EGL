...
namespace EGL.GodotBase.Nodes2D.Utility
{
    public class Path2D : INode
    {
        ...
        public Godot.Path2D Base { get; }

        public Path2D()
        {
            Base = new _Path2D(this);
        }

        private class _Path2D : Godot.Path2D
        {
            public Path2D ClassOwner;

            public _Path2D(Path2D node)
            {
                ClassOwner = node;
