...
namespace EGL.GodotBase.Nodes2D.Render
{
    public class Light2D : INode
    {
        ...
        public Godot.Light2D Base { get; }

        public Light2D()
        {
            Base = new _Light2D(this);
        }

        private class _Light2D : Godot.Light2D
        {
            public Light2D ClassOwner;

            public _Light2D(Light2D node)
            {
                ClassOwner = node;
