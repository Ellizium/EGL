...
namespace EGL.GodotBase.Nodes2D.Render
{
    public class Camera2D : INode
    {
        ...
        public Godot.Camera2D Base { get; }

        public Camera2D()
        {
            Base = new _Camera2D(this);
        }

        public class _Camera2D : Godot.Camera2D
        {
            public Camera2D ClassOwner;

            public _Camera2D(Camera2D node)
            {
                ClassOwner = node;
