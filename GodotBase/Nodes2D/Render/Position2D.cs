...
namespace EGL.GodotBase.Nodes2D.Render
{
    public class Position2D : INode
    {
        ...
        public Godot.Position2D Base { get; }

        public Position2D()
        {
            Base = new _Position2D(this);
        }

        private class _Position2D : Godot.Position2D
        {
            public Position2D ClassOwner;

            public _Position2D(Position2D node)
            {
                ClassOwner = node;
