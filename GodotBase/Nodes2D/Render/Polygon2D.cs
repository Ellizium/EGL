...
namespace EGL.GodotBase.Nodes2D.Render
{
    public class Polygon2D : INode
    {
        ...
        public Godot.Polygon2D Base { get; }

        public Polygon2D()
        {
            Base = new _Polygon2D(this);
        }

        private class _Polygon2D : Godot.Polygon2D
        {
            public Polygon2D ClassOwner;

            public _Polygon2D(Polygon2D node)
            {
                ClassOwner = node;
