...
namespace EGL.GodotBase.Nodes2D.Render
{
    public class Line2D : INode
    {
        ...
        public Godot.Line2D Base { get; }

        public Line2D()
        {
            Base = new _Line2D(this);
        }

        private class _Line2D : Godot.Line2D
        {
            public Line2D ClassOwner;

            public _Line2D(Line2D node)
            {
                ClassOwner = node;
