...
namespace EGL.GodotBase.GUI.Text
{
    public class TextEdit : INode
    {
        ...
        public event Action<int, string> InfoClicked;
        public event Action RequestCompletion;
        public event Action<string, int, int> SymbolLookup;
        public event Action TextChanged;

        public Godot.TextEdit Base { get; }

        public TextEdit()
        {
            Base = new _TextEdit(this);
        }

        protected class _TextEdit : Godot.TextEdit
        {
            public TextEdit ClassOwner;

            public _TextEdit(TextEdit node)
            {
                ClassOwner = node;
                ...
                //TextEdit
                Connect("breakpoint_toggled", this, nameof(BreakpointToggled));
                Connect("cursor_changed", this, nameof(CursorChanged));
                Connect("info_clicked", this, nameof(InfoClicked));
                Connect("request_completion", this, nameof(RequestCompletion));
                Connect("symbol_lookup", this, nameof(SymbolLookup));
            ...
            #region TextEdit
            private void BreakpointToggled(int row)
            {
                ClassOwner.BreakpointToggled?.Invoke(row);
            }
            private void CursorChanged()
            {
                ClassOwner.CursorChanged?.Invoke();
            }
            private void InfoClicked(int row, string info)
            {
                ClassOwner.InfoClicked?.Invoke(row, info);
            }
            private void RequestCompletion()
            {
                ClassOwner.RequestCompletion?.Invoke();
            }
            private void SymbolLookup(string symbol, int row, int colomn)
            {
                ClassOwner.SymbolLookup?.Invoke(symbol, row, colomn);
            }
            private void TextChanged()
            {
                ClassOwner.TextChanged?.Invoke();
            }
            #endregion
        }
    }
