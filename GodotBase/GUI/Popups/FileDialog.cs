...
namespace EGL.GodotBase.GUI.Popups
{
    public class FileDialog : INode
    {
        ...
        public Godot.FileDialog Base { get; }

        public FileDialog()
        {
            Base = new _FileDialog(this);
        }

        protected class _FileDialog : Godot.FileDialog
        {
            public FileDialog ClassOwner;

            public _FileDialog(FileDialog node)
            {
                ClassOwner = node;
                ...
                Connect("popup_hide", this, nameof(PopupHide));

                //Accept Dialog
                Connect("confirmed", this, nameof(Confirmed));
                Connect("cutom_action", this, nameof(CustomAction));

                //FileDialog
                Connect("dir_selected", this, nameof(DirSelected));
                Connect("file_selected", this, nameof(FileSelected));
                Connect("files_selected", this, nameof(FilesSelected));
            }
            ...
            #region Popup
            private void AboutToShow()
            {
                ClassOwner.AboutToShow?.Invoke();
            }
            private void PopupHide()
            {
                ClassOwner.PopupHide?.Invoke();
            }
            #endregion
            ...
            {
                ClassOwner.Confirmed?.Invoke();
            }
            private void CustomAction(string action)
            {
                ClassOwner.CustomAction?.Invoke(action);
            }
            #endregion

            #region FileDialog
            private void DirSelected(string path)
            {
                ClassOwner.DirSelected?.Invoke(path);
            }
            private void FileSelected(string path)
            {
                ClassOwner.FileSelected?.Invoke(path);
            }
            private void FilesSelected(string[] paths)
            {
                ClassOwner.FilesSelected?.Invoke(paths);
            }
            #endregion
        }
    }
