...
namespace EGL.GodotBase.GUI.Buttons
{
    public class ColorPickerButton : INode
    {
        ...
        //ColorPickerButton
        public event Action<Godot.Color> ColorChanged;
        public event Action PickerCreated;
        public event Action PopupClosed;

        public Godot.ColorPickerButton Base { get; }

        public ColorPickerButton()
        {
            Base = new _ColorPickerButton(this);
        }

        protected class _ColorPickerButton : Godot.ColorPickerButton
        {
            public ColorPickerButton ClassOwner;

            public _ColorPickerButton(ColorPickerButton node)
            {
                ClassOwner = node;
                ...
                //Button
                Connect("button_down", this, nameof(ButtonDown));
                Connect("button_up", this, nameof(ButtonUp));
                Connect("pressed", this, nameof(__Pressed));
                Connect("toggled", this, nameof(Toggled));

                //ColorPickerButton
                Connect("color_changed", this, nameof(ColorChanged));
                Connect("picker_created", this, nameof(PickerCreated));
                Connect("popup_closed", this, nameof(PopupClosed));
            }
            ...
            {
                ClassOwner.ButtonDown?.Invoke();
            }
            private void ButtonUp()
            {
                ClassOwner.ButtonUp?.Invoke();
            }
            private void __Pressed()
            {
                ClassOwner.Pressed?.Invoke();
            }
            private void Toggled(bool pressed)
            {
                ClassOwner.Toggled?.Invoke(pressed);
            }
            #endregion

            #region ColorPickerButton
            private void ColorChanged(Godot.Color color)
            {
                ClassOwner.ColorChanged?.Invoke(color);
            }
            private void PickerCreated()
            {
                ClassOwner.PickerCreated?.Invoke();
            }
            private void PopupClosed()
            {
                ClassOwner.PopupClosed?.Invoke();
            }
            #endregion
        }
    }
