...
namespace EGL.GodotBase.GUI
{
    public class Tabs : INode
    {
        ...
        //Tabs
        public event Action<int> RepositionActiveTabRequest;
        public event Action<int> RightButtonPressed;
        public event Action<int> TabChanged;
        public event Action<int> TabClicked;
        public event Action<int> TabClose;
        public event Action<int> TabHover;

        public Godot.Tabs Base { get; }
        ...
        protected class _Tabs : Godot.Tabs
        {
            public Tabs ClassOwner;

            public _Tabs(Tabs node)
            {
                ClassOwner = node;
                ...
                //Tabs
                Connect("reposition_active_tab_request", this, nameof(RepositionActiveTabRequest));
                Connect("right_button_pressed", this, nameof(RightButtonPressed));
                Connect("tab_changed", this, nameof(TabChanged));
                Connect("tab_clicked", this, nameof(TabClicked));
                Connect("tab_close", this, nameof(TabClose));
                Connect("tab_hover", this, nameof(TabHover));
            }
            ...
            #region Tabs
            private void RepositionActiveTabRequest(int indexTo)
            {
                ClassOwner.RepositionActiveTabRequest?.Invoke(indexTo);
            }
            private void RightButtonPressed(int tab)
            {
                ClassOwner.RightButtonPressed?.Invoke(tab);
            }
            private void TabChanged(int tab)
            {
                ClassOwner.TabChanged?.Invoke(tab);
            }
            private void TabClicked(int tab)
            {
                ClassOwner.TabClicked?.Invoke(tab);
            }
            private void TabClose(int tab)
            {
                ClassOwner.TabClose?.Invoke(tab);
            }
            private void TabHover(int tab)
            {
                ClassOwner.TabHover?.Invoke(tab);
            }
            #endregion
        }
    }
