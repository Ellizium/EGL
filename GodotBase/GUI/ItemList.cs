...
namespace EGL.GodotBase.GUI
{
    public class ItemList : INode
    {
        ...
        public event Action<int, Godot.Vector2> ItemRMBSelected;
        public event Action<int> ItemSelected;
        public event Action<int, bool> MultiSelected;
        public event Action NothingSelected;
        public event Action<Godot.Vector2> RMBClicked;

        public Godot.ItemList Base { get; }

        public ItemList()
        {
            Base = new _ItemList(this);
        }

        protected class _ItemList : Godot.ItemList
        {
            public ItemList ClassOwner;

            public _ItemList(ItemList node)
            {
                ClassOwner = node;
                ...
                Connect("rmb_clicked", this, nameof(RMBClicked));
            }
            ...
            {
                ClassOwner.ItemActivated?.Invoke(index);
            }
            ...
            {
                ClassOwner.MultiSelected?.Invoke(index, selected);
            }
