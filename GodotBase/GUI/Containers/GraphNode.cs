...
namespace EGL.GodotBase.GUI.Containers
{
    public class GraphNode : INode
    {
        ...
        public event Action<Godot.Vector2, Godot.Vector2> Dragged;
        public event Action OffsetChanged;
        public event Action RaiseRequest;
        public event Action<Godot.Vector2> ResizeRequest;

        public Godot.GraphNode Base { get; }

        public GraphNode()
        {
            Base = new _GraphNode(this);
        }

        protected class _GraphNode : Godot.GraphNode
        {
            public GraphNode ClassOwner;

            public _GraphNode(GraphNode node)
            {
                ClassOwner = node;
                ...
                Connect("dragged", this, nameof(Dragged));
                Connect("offset_changed", this, nameof(OffsetChanged));
                Connect("raise_request", this, nameof(RaiseRequest));
            ...
            {
                ClassOwner.CloseRequest?.Invoke();
            }
            ...
            {
                ClassOwner.OffsetChanged?.Invoke();
            }
            private void RaiseRequest()
            {
                ClassOwner.RaiseRequest?.Invoke();
            }
