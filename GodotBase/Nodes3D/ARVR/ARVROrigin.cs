...
namespace EGL.GodotBase.Nodes3D.ARVR
{
    public class ARVROrigin : INode
    {
        ...
        public Godot.ARVROrigin Base { get; }

        public ARVROrigin()
        {
            Base = new _ARVROrigin(this);
        }

        protected class _ARVROrigin : Godot.ARVROrigin
        {
            public ARVROrigin ClassOwner;

            public _ARVROrigin(ARVROrigin node)
            {
                ClassOwner = node;
