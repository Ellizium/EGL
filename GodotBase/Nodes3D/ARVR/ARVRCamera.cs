...
namespace EGL.GodotBase.Nodes3D.ARVR
{
    public class ARVRCamera : INode
    {
        ...
        public Godot.ARVRCamera Base { get; }

        public ARVRCamera()
        {
            Base = new _ARVRCamera(this);
        }

        protected class _ARVRCamera : Godot.ARVRCamera
        {
            public ARVRCamera ClassOwner;

            public _ARVRCamera(ARVRCamera node)
            {
                ClassOwner = node;
