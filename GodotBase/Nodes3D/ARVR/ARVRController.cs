...
namespace EGL.GodotBase.Nodes3D.ARVR
{
    public class ARVRController : INode
    {
        ...
        //ARVRController
        public event Action<Godot.JoystickList> ButtonPressed;
        public event Action<Godot.JoystickList> ButtonRelease;
        public event Action<Godot.Mesh> MeshUpdated;

        public Godot.ARVRController Base { get; }

        public ARVRController()
        {
            Base = new _ARVRController(this);
        }

        protected class _ARVRController : Godot.ARVRController
        {
            public ARVRController ClassOwner;

            public _ARVRController(ARVRController node)
            {
                ClassOwner = node;
                ...
                Connect("button_release", this, nameof(ButtonRelease));
                Connect("mesh_updated", this, nameof(MeshUpdated));
            }
            ...
            #region ARVRController
            private void ButtonPressed(int button)
            {
                ClassOwner.ButtonPressed?.Invoke((Godot.JoystickList)button);
            }
            private void ButtonRelease(int button)
            {
                ClassOwner.ButtonRelease?.Invoke((Godot.JoystickList)button);
            }
            private void MeshUpdated(Godot.Mesh mesh)
            {
                ClassOwner.MeshUpdated?.Invoke(mesh);
            }
            #endregion
        }
    }
