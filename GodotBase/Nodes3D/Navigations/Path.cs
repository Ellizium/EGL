...
namespace EGL.GodotBase.Nodes3D.Navigations
{
    public class Path : INode
    {
        ...
        protected class _Path : Godot.Path
        {
            public Path ClassOwner;

            public _Path(Path node)
            {
                ClassOwner = node;
                ...
                //Path
                Connect("curve_changed", this, nameof(CurveChanged));
            }
            ...
            {
                ClassOwner.CurveChanged?.Invoke();
            }
            #endregion
        }
    }
