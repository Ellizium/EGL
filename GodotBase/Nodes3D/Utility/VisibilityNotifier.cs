...
namespace EGL.GodotBase.Nodes3D.Utility
{
    public class VisibilityNotifier : INode
    {
        ...
        //VisibilityNotifier
        public event Action<Godot.Camera> CameraEntered;
        public event Action<Godot.Camera> CameraExited;
        public event Action ScreenEntered;
        public event Action ScreenExited;

        public Godot.VisibilityNotifier Base { get; }

        public VisibilityNotifier()
        {
            Base = new _VisibilityNotifier(this);
        }

        protected class _VisibilityNotifier : Godot.VisibilityNotifier
        {
            public VisibilityNotifier ClassOwner;

            public _VisibilityNotifier(VisibilityNotifier node)
            {
                ClassOwner = node;
                ...
                //VisibilityNotifier
                Connect("camera_entered", this, nameof(CameraEntered));
                Connect("camera_exited", this, nameof(CameraExited));
            ...
            #region VisibilityNotifier
            private void CameraEntered(Godot.Camera camera)
            {
                ClassOwner.CameraEntered?.Invoke(camera);
            }
            private void CameraExited(Godot.Camera camera)
            {
                ClassOwner.CameraExited?.Invoke(camera);
            }
