...
namespace EGL.GodotBase.Nodes3D.Audio
{
    public class Listener : INode
    {
        ...
        public Godot.Listener Base { get; }

        public Listener()
        {
            Base = new _Listener(this);
        }

        protected class _Listener : Godot.Listener
        {
            public Listener ClassOwner;

            public _Listener(Listener node)
            {
                ClassOwner = node;
