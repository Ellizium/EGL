...
namespace EGL.GodotBase.Nodes3D.Render.CSG
{
    public class CSGBox : INode
    {
        ...
        public Godot.CSGBox Base { get; }

        public CSGBox()
        {
            Base = new _CSGBox(this);
        }

        protected class _CSGBox : Godot.CSGBox
        {
            public CSGBox ClassOwner;

            public _CSGBox(CSGBox node)
            {
                ClassOwner = node;
