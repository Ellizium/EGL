...
namespace EGL.GodotBase.Nodes3D.Render.CSG
{
    public class CSGMesh : INode
    {
        ...
        public Godot.CSGMesh Base { get; }

        public CSGMesh()
        {
            Base = new _CSGMesh(this);
        }

        protected class _CSGMesh : Godot.CSGMesh
        {
            public CSGMesh ClassOwner;

            public _CSGMesh(CSGMesh node)
            {
                ClassOwner = node;
