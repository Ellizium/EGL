...
namespace EGL.GodotBase.Nodes3D.Render.Visual
{
    public class GridMap : INode
    {
        ...
        //GridMap
        public event Action<Godot.Vector3> CellSizeChanged;


        public Godot.GridMap Base { get; }

        public GridMap()
        {
            Base = new _GridMap(this);
        }

        protected class _GridMap : Godot.GridMap
        {
            public GridMap ClassOwner;

            public _GridMap(GridMap node)
            {
                ClassOwner = node;
                ...
                //GridMap
                Connect("cell_size_changed", this, nameof(CellSizeChanged));
            }
            ...
            #region GridMap
            private void CellSizeChanged(Godot.Vector3 vector)
            {
                ClassOwner.CellSizeChanged?.Invoke(vector);
            }
            #endregion
        }
    }
