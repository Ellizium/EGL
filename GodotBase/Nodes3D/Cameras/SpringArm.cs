...
namespace EGL.GodotBase.Nodes3D.Cameras
{
    public class SpringArm : INode
    {
        ...
        public Godot.SpringArm Base { get; }

        public SpringArm()
        {
            Base = new _SpringArm(this);
        }

        protected class _SpringArm : Godot.SpringArm
        {
            public SpringArm ClassOwner;

            public _SpringArm(SpringArm node)
            {
                ClassOwner = node;
